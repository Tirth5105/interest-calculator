...
class _HomePageState extends State<HomePage> {
  ...
  void _calculate() {
    ...
    for (int day = 1; day <= days; day++) {
      ...
      results.add({'day': day, 'previous': previous, 'current': current});
    }
    setState(() { _results = results; _calculated = true; });
    FocusScope.of(context).unfocus();
  }

  void _reset() {
    setState(() {
      _results = []; _calculated = false;
      ...
      body: Column(children: [
        Container(
          ...
          child: Column(children: [
            _buildInputField(controller: _capitalController, label: 'Capital (₹)', icon: Icons.currency_rupee, hint: 'e.g. 100'),
            const SizedBox(height: 12),
            _buildInputField(controller: _interestController, label: 'Interest Rate (%)', icon: Icons.percent, hint: 'e.g. 20'),
            const SizedBox(height: 12),
            _buildInputField(controller: _daysController, label: 'Number of Days', icon: Icons.calendar_today, hint: 'e.g. 100'),
        ...
        if (_calculated && _results.isNotEmpty)
          Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
              ...
              _summaryItem('Final (Day ${_results.length})', '₹${_results.last['current'].toStringAsFixed(2)}'),
            ]),
          ),
        if (_calculated) const SizedBox(height: 8),
        if (_calculated)
          Expanded(
                ...
                return Container(
                  ...
                  child: Row(children: [
                    Container(
                      ...
                      child: Center(child: Text('$day', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13))),
                    ),
                    const SizedBox(width: 12),
                    Expanded(child: Text('₹${prev.toStringAsFixed(2)} + ${_interestController.text}% = ₹${curr.toStringAsFixed(2)}', style: const TextStyle(fontSize: 14))),
                    Text('+₹${gain.toStringAsFixed(2)}', style: const TextStyle(color: Color(0xFF2E7D32), fontWeight: FontWeight.bold, fontSize: 13)),
                  ]),
                );
              },
            ),
        ...
        if (!_calculated)
          const Expanded(child: Center(child: Text('Enter values above and\ntap Calculate', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey, fontSize: 16)))),
      ]),
    );
  }

  Widget _buildInputField({required TextEditingController controller, required String label, required IconData icon, required String hint}) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: const BorderSide(color: Color(0xFF1B5E20), width: 2)),
      ),
    );
  }
  ...
  @override
  void dispose() {
    _capitalController.dispose(); _interestController.dispose(); _daysController.dispose();
    super.dispose();
  }
}
